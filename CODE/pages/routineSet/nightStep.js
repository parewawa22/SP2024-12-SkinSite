...
export default function Step() {
  ...
  const getLineWidth = () => {
    if (!rowStepRef.current) return "0px";

    const stepNodes = rowStepRef.current.querySelectorAll(`.${styles.nextStep}`);
    const completedSegments = selectedStep - 1;

    if (completedSegments <= 0 || stepNodes.length < 2) return "0px";

    const first = stepNodes[0].getBoundingClientRect();
    const last = stepNodes[completedSegments].getBoundingClientRect();
  ...
  const stepCategories = {
    1: ["CTG00001"], // cleansing
    2: ["CTG00002"], // Cleanser
    3: ["CTG00003"], // scrub
    4: ["CTG00004"], // Toner
    5: ["CTG00005"], // Essence
    6: ["CTG00010", "CTG00012"], // Eye cream
    7: ["CTG00006", "CTG00007"], // Serum
    8: ["CTG00011", "CTG00013"], // Eye Mask, Mask
    9: ["CTG00008"], // Moisturizer, Cream
    10: ["CTG00014"] // Sleeping Mask
  };
        ...
        <div className={styles.content}>
          <div className={styles.RoutineSet}>
            ...
            <div className={styles.rowStepWithArrow}>
              <button className={styles.arrowButton} disabled={selectedStep === 1} onClick={() => {
              ...
              {/* All step */}
              <div className={styles.rowStep} ref={rowStepRef}>
                  ...
                  <div key={step} className={styles.nextStep}
                    ...
                    <div className={selectedStep >= step ? styles.circle : styles.whNext}>
                      <div className={selectedStep >= step ? styles.whiteCircle : styles.whiteCirclePlus}>
                        <h4 className="stepNumber">{step < 10 ? `0${step}` : `${step}`}</h4>
                      </div>
                    </div>
                    <h4>
                      {
                ...
                return (
                  <div style={{ display: "flex", alignItems: "center" }}>
                    <Link href={`/productInfo?pdid=${selected.pdid}`}>
                      <img src={imagePath} alt={productName} style={{ width: '80px', height: '80px', marginRight: '10px' }}
                        onError={(e) => {
              ...
              currentProducts.map((product) => (
                <div key={product.pd_id} className={styles.productItem}>
                  ...
                  <button disabled={isSaving} onClick={async () => {
                    ...
                    // save state before next step in create routine
                    if (!isEditMode && !stepid) {
                      const nextStep = selectedStep + 1;
                      saveStepProduct({
                        pdid: product.pd_id,
                  ...
                  <div>
                    ...
                    <button onClick={async () => {
                      try {
                        ...
                        alert("✅ Step updated!");
                        router.push(`/routineSet/nightRoutineDetail?routine_id=${routine_id}`);
                      } catch (err) {
                  ...
                  <>
                    ...
                    <button onClick={async () => {
                      try {
                        ...
                        alert("✅ Step added!");
                        router.push(`/routineSet/nightRoutineDetail?routine_id=${routineIdFromQuery}`);
                      }
                      catch (err) {
